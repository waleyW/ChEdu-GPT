...
DB_PATH = r'/home/ubuntu/mnt2/wxy/Ako_GPT/db_1/vector_store.db'
    ...
    if text.lower().startswith('question id is'):
        ...
        try:
            ...
            for entry, question, answer, stored_vector in results:
                if stored_vector is not None:  # 确保向量不为空
                    vector = np.frombuffer(stored_vector, dtype=np.float32)
                    similarity = float(np.dot(query_vector, vector))  # 转换为float
                    print(f"Similarity with {entry}: {similarity}")  # 调试输出
                    similarities.append((similarity, question, answer))
            
            if similarities:  # 确保有结果
                # 找到最相似的问题及其答案
                similarities.sort(key=lambda x: x[0], reverse=True)
                max_similarity = similarities[0][0]
                
                if max_similarity > 0.7:
                    best_question = similarities[0][1]
                    best_answer = similarities[0][2]
                    # 构建包含上下文的prompt
